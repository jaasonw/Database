...
template <typename T>
int index_of_maximal(Array<T>& data, size_t size) {
    int largest = 0;
...
template <typename T>
void insert_item(Array<T>& data, size_t pos, size_t& size, const T& entry) {
...
template <typename T>
void ordered_insert(Array<T>& data, size_t& size, const T& entry) {
    int pos = 0;
    for (size_t i = 0; i < size; i++) {
        if (entry > data[i]) {
            pos = i + 1;
...
template <typename T>
int first_ge(const Array<T>& data, size_t size, const T& entry) {
    for (size_t i = 0; i < size; i++) {
        // if (!(data[i] < entry)) {
        if (data[i] >= entry) {
            return i;
...
template <typename T>
void attach_item(Array<T>& data, size_t& size, const T& entry) {
    data[size++] = entry;
}

// remove the last element in data and place it in entry
template <typename T>
T detach_item(Array<T>& data, size_t& size) {
    T item = data[size - 1];
    data[size] = T();
    data[--size] = T();
    return item;
...
template <typename T>
T delete_item(Array<T>& data, int index, size_t& size) {
    T item = data[index];
...
template <typename T>
void merge(Array<T>& data1, Array<T>& data2, size_t& size1, size_t& size2) {
...
template <typename T>
void split(Array<T>& data1, size_t& size1, Array<T>& data2, size_t& size2) {
    size_t size = size1;
...
// print array data
template <typename T>
void print_array(const Array<T>& data, size_t size, int level = 0,
...
// template <typename T>
// int binary_search(const Array<T>& data, size_t size, const T& item) {
//     if (size >= 1)
// }

// item > all data[i]
template <typename T>
bool is_gt(const Array<T>& data, size_t size, const T& item) {
    for (size_t i = 0; i < size; i++) {
...
// item <= all data[i]
template <typename T>
bool is_le(const Array<T>& data, size_t size, const T& item) {
    for (size_t i = 0; i < size; i++) {
        if (!(data[i] < item) && data[i] != item) {
            return false;
        }
